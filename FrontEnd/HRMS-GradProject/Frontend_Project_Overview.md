...
export class AiService {
  ...
  chat(message: string, mode: number = 0): Observable<AiResponseDto> {
    return this.http.post<any>(`${this.apiUrl}/chat`, { message, mode }).pipe(map((res) => res?.data ?? res));
  }
      ...
      <div class="ai-header-right">
        <div class="token-monitor" *ngIf="tokenStats">
          <div class="token-stats-text">
            <span class="token-count"><i class="bi bi-cpu"></i> {{ tokenStats.usedTokens | number }} / {{ tokenStats.maxTokensPerMinute | number }} TPM</span>
            <span class="token-timer" [class.danger]="tokenStats.secondsUntilReset < 10"><i class="bi bi-clock-history"></i> Resets in {{ tokenStats.secondsUntilReset }}s</span>
          </div>
          <div class="token-progress-bg">
            <div class="token-progress-fill" [style.width.%]="(tokenStats.usedTokens / tokenStats.maxTokensPerMinute) * 100" [class.warning]="(tokenStats.usedTokens / tokenStats.maxTokensPerMinute) > 0.7" [class.danger]="(tokenStats.usedTokens / tokenStats.maxTokensPerMinute) > 0.9"></div>
          </div>
        </div>
      </div>
    </div>
...
<div class="page-container p-4">
  <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
    ...
    <div class="d-flex align-items-center gap-2 flex-grow-1 justify-content-end">
      ...
      <button class="btn btn-outline-success btn-sm rounded-3 text-nowrap shadow-sm p-2 px-3" (click)="exportToExcel()" title="Export to Excel"><i class="bi bi-file-earmark-excel-fill me-1"></i> Export to Excel</button>
      <button class="btn btn-outline-danger btn-sm rounded-3 text-nowrap shadow-sm p-2 px-3" (click)="exportToPDF()" title="Export to PDF"><i class="bi bi-file-earmark-pdf-fill me-1"></i> Export to PDF</button>
      <a routerLink="/employees" class="btn btn-outline-secondary btn-sm rounded-3 text-nowrap shadow-sm p-2 px-3"> <i class="bi bi-arrow-left me-1"></i> Back to Employees </a>
    </div>
  </div>
  ...
  <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
    ...
    <div class="d-flex align-items-center gap-2 flex-grow-1 justify-content-end flex-wrap">
      <div class="input-group shadow-sm" style="max-width: 300px;">
        <span class="input-group-text bg-white border-end-0 text-muted"><i class="bi bi-search"></i></span>
        <input type="text" class="form-control border-start-0 ps-0" placeholder="{{ 'Search employees...' | t }}" [(ngModel)]="searchQuery" (input)="filterRecords()" />
      </div>

      <button class="btn btn-outline-success shadow-sm px-3 px-md-4 py-2 rounded-3 fw-semibold text-nowrap d-flex align-items-center" (click)="exportToExcel()"><i class="bi bi-file-earmark-excel-fill fs-5" [class.me-md-2]="true"></i> <span class="d-none d-md-inline">{{ 'Export' | t }}</span></button>

      <div class="dropdown">
        <button class="btn btn-outline-secondary shadow-sm px-3 py-2 rounded-3 d-flex align-items-center" type="button" data-bs-toggle="dropdown" aria-expanded="false"><i class="bi bi-filter fs-5" [class.me-md-2]="true"></i> <span class="d-none d-md-inline">{{ 'Filter' | t }}</span></button>
      ...
      <div class="login-form-section p-5 d-flex flex-column position-relative justify-content-center">
        ...
        <div class="form-content px-lg-3">
          <div class="mb-5 text-center">
            ...
            <br />
            <p class="text-muted loop-typing-sub" style="font-size: 1.1rem; min-height: 26px;">Access your intelligent HR dashboard</p>
          </div>
          ...
          <div class="row mb-3">
            <div class="col-md-6 d-flex flex-column justify-content-end">
              ...
              <select class="form-select" formControlName="priority">
                ...
                <option value="Urgent">{{ 'Important' | t }}</option>
              </select>
            </div>
            <div class="col-md-6 d-flex flex-column justify-content-end">
...
<div class="page-container p-4">
  <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
    ...
    <div class="d-flex align-items-center gap-2">
      <button class="btn btn-outline-success shadow-sm text-nowrap px-4 py-2 rounded-3 fw-semibold" (click)="exportToExcel()" title="Export to Excel"><i class="bi bi-file-earmark-excel-fill me-1"></i> {{ 'Export to Excel' | t }}</button>
      <button class="btn btn-primary shadow-sm text-nowrap px-4 py-2 rounded-3 fw-semibold" (click)="openAddModal()"><i class="bi bi-plus-lg me-1"></i> {{ 'Add Department' | t }}</button>
    </div>
  </div>
    ...
    <form [formGroup]="employeeForm" (ngSubmit)="onSubmit()">
      ...
      <div class="form-section" style="border-left: 4px solid #f59e0b;">
        <div class="section-header" style="color: #d97706;">
          ...
          <span class="badge ms-2" style="background: #fef3c7; color: #92400e; border: 1px solid #fde68a;">Admin Only</span>
        </div>
        <div class="row g-3">
          <div class="col-12">
            <div class="alert d-flex align-items-center gap-2 mb-3 py-2 px-3" style="background: #fffbeb; border: 1px solid #fde68a; border-radius: 8px; color: #78350f; font-size: 0.875rem;">
...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
  ...
  <div class="d-flex align-items-center gap-2 flex-grow-1 justify-content-end flex-wrap">
    ...
    @if (isAdminOrHR) {
    <div class="d-flex align-items-center gap-2">
      <button class="btn btn-outline-danger px-3 px-md-4 fw-semibold text-nowrap" (click)="exportToPDF()" title="Export to PDF">
        <i class="bi bi-file-earmark-pdf-fill" [class.me-md-2]="true"></i>
        <span class="d-none d-md-inline">{{ 'Export to PDF' | t }}</span>
      </button>
      <button class="btn btn-outline-success px-3 px-md-4 fw-semibold text-nowrap" (click)="exportToExcel()" title="Export to Excel">
        <i class="bi bi-file-earmark-excel-fill" [class.me-md-2]="true"></i>
        <span class="d-none d-md-inline">{{ 'Export to Excel' | t }}</span>
      </button>
      <button class="btn btn-primary px-3 px-md-4 fw-semibold text-nowrap" routerLink="/employee-form" title="Add Employee">
...
<div class="page-container p-4">
  <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
    ...
    <div class="d-flex align-items-center gap-2 flex-grow-1 justify-content-end">
      <div class="input-group shadow-sm" style="max-width: 350px;">
        <span class="input-group-text bg-white border-end-0 text-muted"><i class="bi bi-search"></i></span>
        <input type="text" class="form-control border-start-0 ps-0" placeholder="{{ 'Search by name, ID, or reason...' | t }}" [(ngModel)]="leaveSearchQuery" (input)="filterLeaves()" />
      </div>
      ...
      <button class="btn btn-outline-success px-3 px-md-4 py-2 rounded-3 fw-semibold shadow-sm text-nowrap" (click)="exportToExcel()" title="Export to Excel"><i class="bi bi-file-earmark-excel-fill" [class.me-md-2]="true"></i> <span class="d-none d-md-inline">{{ 'Export to Excel' | t }}</span></button>
      <button class="btn btn-outline-danger px-3 px-md-4 py-2 rounded-3 fw-semibold shadow-sm text-nowrap" (click)="exportToPDF()" title="Export to PDF"><i class="bi bi-file-earmark-pdf-fill" [class.me-md-2]="true"></i> <span class="d-none d-md-inline">{{ 'Export to PDF' | t }}</span></button>

      <button *ngIf="!isAdminOrHR" class="btn btn-primary px-3 px-md-4 py-2 rounded-3 fw-semibold shadow-sm text-nowrap" (click)="openModal()"><i class="bi bi-send-plus" [class.me-md-2]="true"></i> <span class="d-none d-md-inline">{{ 'Request Leave' | t }}</span></button>
    </div>
  </div>
...
export class LeaveComponent implements OnInit {
  ...
  loadLeaves() {
    ...
    const request = this.isAdminOrHR ? this.leaveService.getAllLeaves() : this.leaveService.getMyLeaves();
    ...
    request.subscribe({
      next: (res: any) => {
        ...
        if (this.isAdminOrHR && this.leavesList.length > 0) {
          ...
          const usedAnnualLeavesDays = this.allLeavesList.filter((l: any) => (this.getStatusText(l.status) === "Approved" || this.getStatusText(l.status) === "Pending") && this.getLeaveTypeText(l.leaveType) === "Annual").reduce((acc: number, l: any) => acc + (l.totalDays || 0), 0);
...
<div class="container-fluid py-4">
  <div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="d-flex align-items-center gap-2">
      <button class="btn btn-outline-success shadow-sm rounded-pill px-4" (click)="exportToExcel()" title="Export to Excel"><i class="bi bi-file-earmark-excel-fill me-2"></i> {{ 'Export to Excel' | t }}</button>
      <button *ngIf="isHrOrAdmin" class="btn btn-dark shadow-sm rounded-pill px-4" (click)="openAddModal()"><i class="bi bi-plus-lg me-2"></i> {{ 'Schedule Meeting' | t }}</button>
    </div>
  </div>
...
<div class="page-container p-4">
  <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
    ...
    <div class="d-flex align-items-center gap-2">
      <button class="btn btn-outline-success shadow-sm text-nowrap px-4 py-2 rounded-3 fw-semibold me-2" (click)="exportToExcel()" title="Export to Excel"><i class="bi bi-file-earmark-excel-fill me-1"></i> {{ 'Export to Excel' | t }}</button>
      <button class="btn btn-outline-danger shadow-sm text-nowrap px-4 py-2 rounded-3 fw-semibold" (click)="exportToPDF()" title="Export to PDF"><i class="bi bi-file-earmark-pdf-fill me-1"></i> {{ 'Export to PDF' | t }}</button>
      @if (isAdminOrHR) {
      <button class="btn btn-primary shadow-sm text-nowrap px-4 py-2 rounded-3 fw-semibold" (click)="openAddModal()"><i class="bi bi-plus-lg me-1"></i> {{ 'Add Adjustment' | t }}</button>
      }
    </div>
  </div>
      ...
      <table class="table table-hover align-middle mb-0">
        <thead class="bg-light">
          <tr>
            ...
            <th class="py-3 px-4 text-muted small text-uppercase fw-semibold border-bottom-0">Date</th>
        ...
        <tbody class="border-top-0">
          ...
          <tr>
            ...
            <td class="py-3 px-4 text-muted">{{ adj.date | date:'mediumDate' }}</td>
...
export class PayrollAdjustmentsComponent implements OnInit {
  ...
  loadAdjustments() {
    ...
    const request = this.isAdminOrHR ? this.adjustmentService.getAll(this.pageNumber, this.pageSize) : this.adjustmentService.getMyAdjustments(this.pageNumber, this.pageSize);
...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
  ...
  <div class="d-flex align-items-center gap-2">
    <button class="btn btn-outline-success shadow-sm fw-bold px-3 px-md-4" (click)="exportToExcel()" title="Export to Excel"><i class="bi bi-file-earmark-excel-fill" [class.me-md-1]="true"></i> <span class="d-none d-md-inline">{{ 'Export to Excel' | t }}</span></button>
...
<div class="page-container p-4">
  <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
    ...
    <div class="d-flex align-items-center gap-2 flex-grow-1 justify-content-end flex-wrap">
      <div class="input-group shadow-sm" style="max-width: 350px; min-width: 200px; flex: 1;">
        <span class="input-group-text bg-white border-end-0 text-muted"><i class="bi bi-search"></i></span>
        <input type="text" class="form-control border-start-0 ps-0" placeholder="{{ 'Search by name, ID, or amount...' | t }}" [(ngModel)]="salarySearchQuery" (input)="filterSalaries()" />
      </div>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      ...
      <div class="modal-body p-4">
        <form #salaryForm="ngForm">
          ...
          <div class="row mb-4">
            <div class="col-6">
              <label class="form-label fw-semibold text-success small">Allowances (+) <span class="text-danger">*</span></label>
            ...
            <div class="col-6">
              <label class="form-label fw-semibold text-danger small">Deductions (-) <span class="text-danger">*</span></label>
...
export class SalaryComponent implements OnInit {
  ...
  loadSalaries() {
    this.isLoading = true;
    const request = this.isAdminOrHR && this.isViewingAll ? this.salaryService.getAllSalaries() : this.salaryService.getMySalaries();
